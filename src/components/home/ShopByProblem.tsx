...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  weed: Leaf, pest: Bug, disease: ShieldAlert, nutrition: Droplets, seed: Sprout, growth: TrendingUp,
};
    ...
    <section className="py-12 md:py-16 relative">
      ...
      <div ref={ref} className="container reveal-section relative">
        <div className="text-center mb-8">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-accent/60 mb-2">{t("section.findSolutions")}</p>
          <h2 className="text-2xl md:text-3xl font-extrabold text-foreground">
            {t("section.shopByProblem")} <span className="text-gradient-gold">{t("section.problem")}</span>
          </h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 stagger-children revealed">
          {problemCategories.map(cat => {
            ...
            return (
              <Link key={cat.slug} to={`/products?problem=${cat.slug}`} className="group premium-card text-center p-5 cursor-pointer">
                ...
                <p className="text-sm font-bold text-foreground mb-0.5 group-hover:text-primary transition-colors">
                  {language === "ru" ? cat.nameRoman : cat.name}
                </p>
                <p className="text-[10px] text-muted-foreground/60 mb-1" dir="rtl">{cat.nameUrdu}</p>
                <p className="text-[10px] text-muted-foreground font-medium">{cat.count} {language === "ru" ? "products" : "products"}</p>
                <div className="mt-2 flex items-center justify-center gap-1 text-primary/0 group-hover:text-primary/80 transition-all duration-200 text-[10px] font-semibold">
                  {t("section.explore")} <ArrowRight className="h-3 w-3" />
                </div>
              </Link>
            );
          })}
