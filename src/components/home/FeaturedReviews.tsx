...
function StatCounter({ target, label, suffix = "+" }: { target: number; label: string; suffix?: string }) {
  ...
  return (
    <div ref={ref} className="text-center">
      <p className="text-2xl md:text-3xl lg:text-4xl font-extrabold text-gradient-gold leading-none">{count.toLocaleString()}{suffix}</p>
      ...
      <div className="container relative">
        <div ref={ref} className="reveal-section mb-12">
          <div className="text-center mb-8">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-primary/60 mb-2">{t("section.ourImpact")}</p>
            <h2 className="text-2xl md:text-3xl font-extrabold text-foreground">
              {t("section.trustedBy")} <span className="text-gradient-green">{t("section.farmersAcross")}</span>
            </h2>
          </div>
          <div className="premium-card rounded-2xl p-6 md:p-10 max-w-2xl mx-auto">
            <div className="grid grid-cols-3 gap-4">
              <StatCounter target={stats.totalFarmers} label={t("section.happyFarmers")} />
              <StatCounter target={stats.productsDelivered} label={t("section.productsDelivered")} />
              <StatCounter target={stats.citiesCovered} label={t("section.citiesCovered")} />
            </div>
          </div>
        </div>
        <div ref={reviewsRef} className="reveal-section">
          <div className="text-center mb-6">
            <h3 className="text-xl md:text-2xl font-extrabold text-foreground">
              {t("section.whatFarmersSay")} <span className="text-gradient-gold">{t("section.farmersSay")}</span>
            </h3>
          </div>
                ...
                <div className="flex items-center justify-between border-t border-border/50 pt-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground flex items-center gap-0.5"><MapPin className="h-2.5 w-2.5" /> {review.location}</p>
                    </div>
                  </div>
                  {review.verified && (
                    <div className="flex flex-col items-end">
                      <div className="flex items-center gap-1 text-primary"><BadgeCheck className="h-3.5 w-3.5" /><span className="text-[9px] font-bold">{t("common.verifiedPurchase")}</span></div>
                      {review.orderId && <span className="text-[8px] text-muted-foreground">{t("common.order")} #{review.orderId}</span>}
                    </div>
                  )}
                </div>
              </div>
